...
const getPDFImages = async (pdfPath, pagesToProcess) => {
...
const html2chunks = async (
  ...
  const chunks = await textSplitter.splitText(html);
...
/**
 * Construit un prompt à partir d'une liste d'articles, avec gestion du format et de la limite de tokens.
 *
 * @param {ArticleShape[]} articles - Liste des articles sources utilisés pour générer le prompt.
 * @param {"text"|"json"} [format="text"] - Format de sortie attendu :
 *   - `"text"` → prompt en texte brut
 *   - `"json"` → prompt structuré en JSON
 * @param {number} [maxTotalTokens] - Nombre maximal de tokens autorisés dans le prompt.
 *   Si non défini, aucun contrôle n'est appliqué.
 * @param {function} [dbg=() => null] - Fonction de débogage optionnelle appelée
 *   avec des informations intermédiaires (signature `(info: any) => void`).
 * @returns {string} - Le prompt généré, soit en texte brut (string),
 *   soit en objet JSON stringifié selon le format choisi.
 */
function buildPromptFromArticlesMulti(
  ...
  for (const article of articles) {
    const {kind, title, sourceId, description, finalScore} = article;

    let articleBlock = '';
    if (format === 'json') {
      articleBlock = JSON.stringify({
        kind,
        title,
        url: sourceId.startsWith('http') ? sourceId : null,
        finalScore,
        description,
      });
    } else {
      articleBlock += `# ${title}\n\n`;
      articleBlock += `**score :** ${finalScore}\n\n`;
      articleBlock += `\`\`\`html\n${description}\n\`\`\`\n\n`;
    }
...
function splitThinkTags(input) {
  ...
  // Extraire toutes les sections <think>...</think>
  while ((match = thinkRegex.exec(input)) !== null) {
    thinks.push(match[1].trim());
  }

  // Supprimer les balises et leur contenu du texte
  cleanText = cleanText.replace(thinkRegex, '').trim();

  return {
    thinks, // Tableau avec le contenu des balises <think>
    cleanText, // Texte sans balises <think>
  };
}
