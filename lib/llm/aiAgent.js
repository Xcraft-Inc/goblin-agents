...
const LLMProviders = require('./providers.js');
const {OCR_SYSTEM_PROMPT, getPDFImages, splitThinkTags} = require('./utils.js');
const Logger = require('../logger.js');

const CONVERSATION_TITLE_PROMPT = `Create a concise and informative title that summarizes the main topic of the conversation.
Use the same language as the conversation. If several languages are used, use the language of the most recent user message.
Use no more than eight words.
Return only the title, without quotation marks, Markdown, a prefix, or an explanation.`;
...
function cleanText(text) {
  ...
  const {cleanText} = splitThinkTags(text);
  return cleanText;
}
...
class AiAgentLogic extends Elf.Archetype {
  ...
  state = new AiAgentState({
    ...
    otherModels: defaultValues.otherModels,
    messages: {},
  ...
  useOtherModel(model) {
    this.state.otherModels.deleteByValue(model);
    this.state.otherModels.push(this.state.model);
...
class AiAgent extends Elf {
  ...
  async _loadProvider() {
    ...
    const Instance = LLMProviders.get(this.state.provider);
  ...
  async useOtherModel(model) {
    if (!this.state.otherModels.includes(model)) {
      return false;
    }
    this.logic.useOtherModel(model);
    return true;
  }

  async getModelList() {
    return [this.state.model, ...this.state.otherModels];
  }

  async gen(prompt, contextId = 'default') {
    const logger = Logger.get(contextId, this);
    ...
    logger.push('aiAgent.gen', {
      id: this.id,
      model: this.state.model,
      options: Object.entries(options).reduce(
        (out, opt) => `${out} ${opt[0]}=${opt[1]}`,
        ''
      ),
      reasoning: this.state.reasoning,
      format: format || 'N/A',
  ...
  async readPDFpages(pdfPath, pages) {
    await this._mutex.lock(this.id);
    this.quest.defer(() => this._mutex.unlock(this.id));

    await this._loadProvider();

    const images = await getPDFImages(pdfPath, pages);
    const descriptions = [];
      ...
      const messages = [
        {
          ...
          content: [
            {
              type: 'text',
              text: this.state.prompt || OCR_SYSTEM_PROMPT,
            },
            {
              type: 'image_url',
              image_url: {
  ...
  async chat(contextId, question, userDesktopId) {
    ...
    logger.push('aiAgent.chat', {
      id: this.id,
      model: chatCompletionRequest.model,
      options: Object.entries(chatCompletionRequest.options).reduce(
        (out, opt) => `${out} ${opt[0]}=${opt[1]}`,
        ''
      ),
      question,
      reasoning: result.message?.reasoning,
      callTools: !!result.message.tool_calls?.length,
    });
    ...
    const res = cleanText(result.message.content);
    if (typeof res === 'string') {
      //keep agent response
      this.logic.chat(contextId, 'assistant', res);
    }
  ...
  async callTools(contextId, message, userDesktopId) {
    ...
    for (const tool of tool_calls) {
      ...
      try {
        ...
        this.log.dbg(`🔧 Tool error : ${err.message}`);
      }
    }
    return true;
  }
  ...
  /**
   * Generates a short title for an existing conversation without changing it.
   * @param {string} contextId
   * @returns {Promise<string>}
   */
  async generateConversationTitle(contextId) {
    const contextMessages = this.state.messages[contextId] || [];
    const prompt = Array.from(contextMessages)
      .map((m) => m._state.toJS())
    ...
    const title = await this._provider.gen({
      model: this.state.model,
      prompt,
  ...
  async getBaseSettings() {
    const {provider, host, headers, model, otherModels} = this.state.toJS();
    return {provider, host, headers, model, otherModels};
  }
