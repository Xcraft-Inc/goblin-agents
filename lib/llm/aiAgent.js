...
function vectorToSqlLiteral(vector) {
  const buffer = Buffer.from(vector.buffer); // Utilise directement le buffer natif du Float32Array
...
class AiAgent extends Elf {
  ...
  async _loadProvider() {
    ...
    this._provider = new Instance(this.state.host, this.state.headers);
  ...
  async embed(rawText) {
    ...
    return vectorToSqlLiteral(vectors);
  }
  ...
  async embedInBatch(rawTexts = []) {
    ...
    return res.map((v) => vectorToSqlLiteral(v));
  }
